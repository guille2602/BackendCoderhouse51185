...
import {
    ...
    generateProductErrorParams,
} from "../services/errors/productErrorsInfo.js";
...
class ProductController {
    ...
    async updateProduct(req, res, next) {
        try {
            ...
            const { product } = await productService.readProduct(
                req.params.pid
            );
            const productOwner = product.owner._id;
            const user = await userService.getUser({ email: req.user.email });
            if (
                productOwner.toString() !== user._id.toString() &&
                !req.session?.admin
            ) {
                return res.status(403).send({
                    status: "failed",
                    description:
                        "El usuario no puede editar productos que no le pertenecen",
                    payload: null,
                });
            }
            const OjbIdRegEx = /^[0-9a-fA-F]{24}$/;
    ...
    async deleteProduct(req, res, next) {
        try {
            ...
            const user = await userService.getUser({ email: req.user.email });
            if (
                productOwnerId.toString() !== user._id.toString() &&
                !req.session?.admin
            ) {
                return res.status(403).send({
                    status: "failed",
            ...
            if (status == 200) {
                req.logger.info("Se ha borrado un producto");
                const prodsList = await productService.readProducts();
                io.emit("updatelist", prodsList);
                if (!req.user?.admin && user.role === "premium") {
                    const productOwner = await userService.getUser({
                        _id: productOwnerId,
                    });
                    const htmlContent = `Se ha eliminado el producto "<b>${product.title}</b>" de la tienda, 
                        si cree que ha sido un error contacte a un administrador`;
